...
def get_repo_names(org_name: str, repo_list: list, repo_regex: str,
                   repo_topic: str, ignore_repos: list) -> list:
...
def get_file_paths(repo_path: str, target: str) -> list:
    ...
    raw_result = subprocess.run(["ag", "-l", target],
                                cwd=repo_path,
                                capture_output=True)
...
def update_repo(org_name: str, repo_name: str, work_dir: str, branch_name: str,
    ...
    for file_path in file_paths:
        update_file(os.path.join(repo_path, file_path), target, replacement)
    ...
    if pr:
        return create_pr(repo, remote_repo, repo_name, base_branch,
...
def main(org_name: str, repo_list: list, repo_regex: str, repo_topic: str,
         ignore_repos: list, branch_name: str, commit_message: str,
         target_file: str, target: str, replacement: str, work_dir: str,
         pr: bool):
    results = []
    ...
    if repo_list and (repo_regex or repo_topic):
        raise ValueError(
            "It's an error to specify a repo list and a regex / topic!")

    if branch_name == "master" or branch_name == "main":
        raise ValueError(
            "--branch-name is the name of your topic branch, it shouldn't be %s!"
            % branch_name)

    print("Gathering list of repos, this may be slow if the organization owns "
          "a lot of repos.")
    repo_names = get_repo_names(org_name, repo_list, repo_regex, repo_topic,
                                ignore_repos)
    ...
    for repo_name in repo_names:
        result_paths = update_repo(org_name, repo_name, work_dir, branch_name,
                                   commit_message, target_file, target,
                                   replacement, pr)
...
def parser():
    ...
    parser.add_argument("--repo-topic",
    ...
    parser.add_argument("--branch-name",
                        required=True,
                        help="Name to create the branch with")
    ...
    parser.add_argument(
        "--work-dir",
        help=
        "Path where the git clones should live. WARNING: this is going to run git reset --hard, so dont pick the same dir that you use for manual edits!"
    )
    parser.add_argument(
        "--pr",
        action="store_true",
...
if __name__ == "__main__":
    ...
    main(args.org_name, args.repo_list, args.repo_regex, args.repo_topic,
         args.ignore_repos, args.branch_name, args.commit_message,
         args.target_file, args.target, args.replacement, args.work_dir,
         args.pr)
